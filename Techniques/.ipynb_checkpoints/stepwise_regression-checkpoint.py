...
def _PSE(predictions, targets, p):
    ...
    s_max = np.sum(np.square(targets))/N
...
def evaluate(Model, inputs, target, showPlots = True):
    ...
    confidenceBounds = 1.96/np.sqrt(N)*e_sig
    ...
    if showPlots is not False:
        ...
        ax.plot(index, np.ones(index.shape)*e_sig, color = 'tab:red', linestyle='--', label = r'$1-\sigma$ bounds')
        ax.plot(index, -1*np.ones(index.shape)*e_sig, color = 'tab:red', linestyle='--')
        ...
        ax3.bar(coeffIndex, np.array(Model['Parameters']).reshape(coeffIndex.shape), color='royalblue', label='Polynomial coefficients')
        ...
        ax3.set_xticklabels(labels=Model['Regressors'])
